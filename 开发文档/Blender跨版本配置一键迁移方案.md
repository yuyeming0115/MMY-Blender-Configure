...
>
> **2026-08-18 v1.3.0 更新**：本文档描述的流程已升级为"两阶段预检迁移"，要点如下
> （详见《配置管理体验重构与备份体系需求方案》与《v1.3.0真机测试列表》）：
>
> - 迁移改两阶段：只读预检 → 确认页（版本/磁盘/插件兼容预测/目标占用）→ 确认后执行
> - 迁移组件按 🟢安全/🟡谨慎/🔴高风险 分级，启动脚本需勾选风险确认
> - 一份快照支持多目标批量迁移（`execute_*_multi`，逐目标独立备份回滚）
> - 配置包选中即展示 manifest 摘要，检测到本机更高版本时提示重新导出（M4）
> - 产物统一命名 `MMY_Backup_{Portable|Profile}_v{版本}_{时间}.zip`，Portable zip 内含 manifest.json
> - 迁移报告同时输出 HTML；恢复入口改为「备份记录」列表（含解压恢复 Portable 备份）
> - 降级（高版本→低版本）按设计拒绝：降级 = 恢复该版本自己的版本化备份
> - 插件启动 5 秒后自动清理 >24h 迁移残留目录；维护区可手动清理
